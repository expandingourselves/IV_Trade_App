...
if run:
    ...
    # Expected Move Information Section
    if expected_move:
        ...
        with col1:
            st.metric(
                label="Expected Move",
                value=expected_move,
                help="Based on nearest expiry straddle price"
            )
        
        try:
            move_percentage = float(expected_move.strip("%"))
            move_dollar = price * (move_percentage / 100)
            
            with col2:
                st.metric(
                    label="Upside Target",
                    value=f"${price + move_dollar:,.2f}",
                    delta=f"+${move_dollar:,.2f}"
                )
            ...
            **Expected Move Interpretation:**
            - The options market is pricing in approximately a **{expected_move}** move by the nearest expiration
            - This represents roughly **${move_dollar:,.2f}** in either direction
            - Upside target: **${price + move_dollar:,.2f}**
            - Downside target: **${price - move_dollar:,.2f}**
            
            *This is based on the cost of an at-the-money straddle and represents the break-even points for that strategy.*
            """)
        except:
            st.info("Expected move calculated from straddle pricing - represents market's implied volatility expectations.")
    ...
    st.info(
        ...
        "- Expected Move shows the market's implied volatility expectation based on straddle pricing."
    )
